...
async function loadLatestApacheByPatient(patients, monthKey) {
  const { start, end } = monthRange(monthKey);
  ...
  const map = new Map();           // pid -> latest score
  for (const s of scores) {
    if (!map.has(s.pid)) map.set(s.pid, s);
  }
  return map;
}
...
async function calcApacheRates(monthKey, department) {
  ...
  const scoreMap = await loadLatestApacheByPatient(patients, monthKey);
  ...
  for (const p of patients) {
    ...
    if (!s) { notScored++; detail.notScored.push(p._id); continue; }
...
async function calcOrderHitOnIcu(monthKey, department, keyword) {
  ...
  const hit = new Set(orders.map(o => o.mrn));
  return { num: [...hit].length, denom: patients.length };
}
...
module.exports = {
  // 算法实现
  calcApacheRates, calcMortality, calcShockBundle, calcDVT,
  calcExtubation, calc48hReturn,
  calcShockUltrasound, calcShockHemodynamic, calcARDS,
  calcEN48h, calcPain, calcSedation, calcRescue, calcBrainInjury,
  readQualityData,
  // 工具
